		...
		<section class="me-4">
			...
			<form action="StudentUpdateexecute.action" method="get">
				<div class="text-align:">
					...
					<input type="text" name="ent_year" value="${student.getEntyear()}"
						readonly
				...
				<div class="text-align:">
					<div class="text-align:">
						<label class="form-label" for="student-f3-select">氏名</label> <input
							type="text" name="name" value="${student.getName()}"
					...
					<div class="text-align:">
						<label class="form-label" for="student-f4-select">クラス</label> <select
							...
							<c:forEach var="num" items="${class_num_set}">
								...
								<option value="${num}"
									<c:if test="${num.contains(student.getClassNum())}">selected</c:if>>${num}</option>
							</c:forEach>
						</select>
					</div>
					...
					<div class="text_align">
						<label class="form-check-label" for="student-f5-check">在学中
							...
							class="form-check-input" type="checkbox" id="student-f5-check"
							name="si_attend" value="t" style="margin-bottom: 10px;"
							<c:if test="${student.getIsAttend()}">checked</c:if> /> <c:if
								test="${!empty f5}">checked</c:if>
						</label>
